            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (내용 생성 완료, 하지만 전체 섹션이 아직 완료되지 않을 수 있음) -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // DOM 업데이트 후 버튼 위치가 동일하게 유지되도록 스크롤 위치 조정
...
const toolConfig = {
  'insight_forge': {
    name: 'Deep Insight',
    color: 'purple',
    icon: 'lightbulb' // 전구 아이콘 - 인사이트를 상징
  },
  'panorama_search': {
    name: 'Panorama Search',
    color: 'blue',
    icon: 'globe' // 지구 아이콘 - 전경 검색을 상징
  },
  'interview_agents': {
    name: 'Agent Interview',
    color: 'green',
    icon: 'users' // 사용자 아이콘 - 대화를 상징
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
    icon: 'zap' // 번개 아이콘 - 빠름을 상징
  },
  'get_graph_statistics': {
    name: 'Graph Stats',
    color: 'cyan',
    icon: 'chart' // 차트 아이콘 - 통계를 상징
  },
  'get_entities_by_type': {
    name: 'Entity Query',
    color: 'pink',
    icon: 'database' // 데이터베이스 아이콘 - 엔티티를 상징
  }
}
  ...
  try {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        // 플랫폼 폴백 로직 (하위 호환성: 하나의 플랫폼 마크만 있는 경우)
        if (!twitterMatch && redditMatch) {
          // Reddit 답변만 있는 경우, 비점유 텍스트일 때만 기본 표시로 복사
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-tabs' }, [
        ...
        props.result.subQueries.length > 0 && h('button', {
          ...
          h('span', { class: 'tab-label' }, `서브쿼리 (${props.result.subQueries.length})`)
        ])
      ]),
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, '시간순 기억에서 연관된 최신 핵심 사실'),
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, '드리프트 쿼리로 생성된 분석 서브쿼리'),
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          ...
          ) : h('div', { class: 'empty-state' }, '역사 기억 없음'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // 각 질문-답변 쌍에 대한 독립적인 플랫폼 선택 상태 유지
    ...
    // 특정 질문에 두 플랫폼 답변이 있는지 확인 (점유 텍스트 필터링)
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // 두 플랫폼 모두 실제 답변(비점유 텍스트)이 있고 내용이 다른 경우
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // 듀얼 플랫폼 전환 버튼 (실제 두 플랫폼 답변이 있을 때만 표시)
...
const renderMarkdown = (content) => {
  ...
  // 앞의 2레벨 제목(## xxx) 제거 - 섹션 제목이 외부 레이어에 이미 표시됨
  ...
  // 블록 레벨 요소 앞뒤 <br> 태그 정리
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // <p><br>이 블록 레벨 요소 바로 앞에 오는 경우 정리 (불필요한 빈 줄 발생)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // 연속 <br> 태그 정리
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // 블록 레벨 요소 뒤 단락 시작 태그 앞 <br> 정리
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // 비연속 순서 목록 번호 수정: 단일 <ol>이 단락 내용으로 분리될 때 번호를 증가 유지
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // 로딩 상태 확실히 제거
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // 작업 완료 시 상단으로 스크롤; 아니면 최신 로그를 따라 하단으로 스크롤
            if (isComplete.value) {
...
const extractFinalContent = (response) => {
  ...
  // ## 또는 # 또는 >로 시작하는 경우 직접적인 마크다운 내용일 수 있음
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // 내용이 길고 마크다운 형식을 포함하는 경우 사고 과정 제거 후 반환 시도
...
.section-number {
  ...
  font-size: 16px;
  color: #9CA3AF; /* 진회색, 상태에 따라 변하지 않음 */
  font-weight: 500;
}
...
/* Console Logs - Step3Simulation.vue와 동일하게 유지 */
.console-logs {
  background: #000;
  color: #DDD;
  padding: 16px;
