          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- 일반 폴백: 알 수 없는 유형 또는 위에서 처리되지 않은 content 보유 -->
...
const props = defineProps({
  ...
  reportSettings: {
    type: Object,
    default: () => ({ maxSections: 5, maxToolCalls: 5, maxTokens: 4096 })
  }
})
...
const phase = ref(0) // 0: 미시작, 1: 실행 중, 2: 완료
const isStarting = ref(false)
const isStopping = ref(false)
const startError = ref(null)
const runStatus = ref({})
const allActions = ref([]) // 전체 동작 (증분 누적)
const actionIds = ref(new Set()) // 중복 제거용 동작 ID 집합
const scrollContainer = ref(null)

// Computed
// 시간순으로 동작 표시 (최신이 맨 아래, 즉 하단)
...
// 시뮬레이션 경과 시간 포맷 (라운드 수와 라운드당 분 수로 계산)
...
const doStartSimulation = async () => {
  if (!props.simulationId) {
    addLog('오류: simulationId 없음')
    return
  }

  // 먼저 모든 상태 초기화하여 이전 시뮬레이션의 영향을 받지 않도록 함
  resetAllState()
...
// 각 플랫폼의 이전 라운드 추적, 변화 감지 및 로그 출력에 사용
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 추가 확인: 백엔드가 runner_status를 업데이트하지 못했지만 플랫폼이 완료를 보고한 경우
...
const checkPlatformsCompleted = (data) => {
  ...
  // 하나 이상의 플랫폼이 완료된 경우, 모든 활성화된 플랫폼이 완료되었는지 확인
  // actions_count로 플랫폼 활성화 여부 판단 (count > 0 이거나 running이 true였던 경우)
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 자동 스크롤 하지 않음, 사용자가 자유롭게 타임라인을 볼 수 있도록 함
      // 새 동작은 하단에 추가됨
    }
  } catch (err) {
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('보고서 생성 요청이 전송됨, 잠시 기다려 주세요...')
    return
  }
  ...
  try {
    const res = await generateReport({
      ...
      max_sections: props.reportSettings.maxSections,
      max_tool_calls: props.reportSettings.maxToolCalls,
      max_tokens: props.reportSettings.maxTokens,
    })
